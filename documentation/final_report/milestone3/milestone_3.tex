...
\begin{abstract}
    This report presents a summary of all the planning and execution of the LowTech GmbH Cloud Transformation Project hosted at https://github.com/Helly2010/Cloud-Computing-Project.
    ...
    The report also discusses future scalability strategies, ensuring that the application can handle different types of loads and makes sense in a cloud environment.
    Finally, the report provides a summary of the lessons learned and interesting findings while using the tools and techniques mentioned.
...
Initially relying on sales representatives, the company transitioned to an online store a few years ago, which has now become its primary selling platform.
...
\begin{itemize}
    ...
    \item \textbf{Manual Processes \& High Cost}:
          ...
          Replacement of hardware is not only tedious but also financially burdensome due to the high costs of new hardware.
...
\begin{itemize}
    \item \textbf{Obsolete Hardware/OS \& Runtime Environments}:
          Many components of the current infrastructure are based on very old hardware and outdated operating systems such as
          ...
          MySQL 5.5/5.7, PHP 5.3, Firefox 3.6, etc. which makes this whole infrastructure more susceptible to failure.
    ...
    \item \textbf{Manual Maintainance}:
          ...
          This increases the possibility of human errors, leads to longer downtime, and reduces overall reliability.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Basic Windows Firewall}:
          ...
          Without additional tools, centralized administration makes it impossible to maintain consistent security rules across all 36 client devices (17 clients and 19 laptops).
...
As detailed in the analysis the infrastructure was becoming quite expensive and inefficient given the company's objectives.
...
\begin{itemize}
    ...
    \item Talk with server space providers, we need to establish a contract and negotiate prices and SLOs based on the server requirements
...
Once the client for the project decided to migrate to a public cloud service provider, we developed a new strategy.
...
We proposed a new standard for this kind of application:
...
\begin{itemize}
    ...
    \item \textbf{Future-Proofing and Cloud-Native Development:}
          The Webshop follows cloud-native best practices such as infrastructure as code, and continuous deployments.
\end{itemize}

\section{Application Design}
\subsection{Architectural Overview}
As requested by the project. This application will have 3 tiers. The presentation or user interface, the backend or business logic, and the storage tier.
By our plan in milestone II, we will deploy each tier in a different Azure service tailored to its needs.
Each of the services provides autoscaling features and load balancing. Ensuring that we are leveraging the elasticity characteristics of the cloud.

\subsubsection{Presentation-Tier (Frontend) - User Interface (UI)}

It is important to note that the majority of the presentation layer was forked from https://github.com/DebasmitaMallick/React-e-Commerce-Website\ref{Webshop}.
...
\begin{enumerate}
    \item Navigation \& Routing
          Users can navigate between different sections of the application, such as viewing product lists, accessing product details, and managing their shopping cart. React Router enables seamless client-side navigation.
          \begin{itemize}
              ...
              \item Navigation Links
                    \begin{itemize}
                        \item On the homepage (Home.js component), displaying a list of products. Each product has an image and name that users can click on. When clicked, the app navigates to the product detail page using the navigate(/product/\${prod.id}) function
                    \end{itemize}
          \end{itemize}
    \item API Communication
          \begin{itemize}
              ...
              \item API communication in the app is responsible for sending and receiving data from the backend server. This includes fetching product data to populate the catalog and creating orders after checkout.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../diagrams/component_diagram.drawio.png}
                    ...
                    \begin{itemize}
                        ...
                        \item Key Attributes:
                              \begin{itemize}
                                  ...
                                  \item status: Enum representing the order state (e.g., ''active'', ''canceled'').
...
divided, and prioritized all the necessary development and documentation tasks. Internally two teams were formed, frontend and backend.
Each works in a different layer of the webshop. The backend team was also in charge of the data layer just for convenience.
The most important tasks were to upgrade the existing UI code to fit the new requirements and integrate with the backend, and to develop the backend.
...
During the development timeline, we scheduled weekly meetings to report progress and address concerns with the development environment.

For example, we encountered issues while running the new `psycopg` library in asynchronous mode in Windows. Which caused some issues while configuring the development environment.
This could have been an opportunity to build a docker image to standardize our development environments, but given the timeline, we decided against it and changed to `asyncpg` in Windows \\
...
The building block view emphasizes the backend since we have provided a component diagram for the front end.\\
...
So far we have not set up any CI implementation for the infrastructure deployment, only for the code changes, since the functionality was provided by Azure.

In the following code snippets, we can see the specific infrastructure template we used for our deployment.
...
\begin{itemize}
    ...
    \item Creation of a virtual network to communicate the database and backend. This is required by the Postgresql flexible server
    ...
    \item Creation of the DB, frontend, and backend modules. All necessary code is included in the /infra folder of the repository.
...
Once we had all resources created in Azure, the team had to perform further manual configurations to enable the CI pipelines in Azure web apps and static websites.
\begin{itemize}
    \item To enable CI for the backend we used the provided Azure functionality which created a GitHub actions file that packages and uploads all changes once they are merged into the main
    \item To enable CI for the front end we used the provided Azure functionality, which essentially provides the same features as in the backend.
    \item We also created a standalone virtual machine with access to the database, in this virtual machine we ran all migrations and seeded the database so all data was available for testing.\\
\end{itemize}
...
\begin{itemize}
    ...
    \item Some functionalities are not available for the student subscription, so we had to change availability zones until we got one that was allowed
    \item By default our applications don't have any autoscaling configured, given the choice of SKUs and to save costs.
          Nevertheless, the functionality can be enabled by changing one configuration in the dashboard
\end{itemize}
...
\begin{longtable}{|c|>{\raggedright}p{4.8cm}|p{6cm}|>{\centering\arraybackslash}p{2cm}|}
    ...
    TC-005                & Filter by Category                         & Products are filtered correctly by the selected category. & Pass, see demo video                          \\ \hline
    TC-006                & Search Product by Name or Category         & Products matching search is shown correctly.              & Pass, see demo video                          \\ \hline
    ...
    TC-012                & Remove the product from the cart           & Product is removed from the cart immediately.             & Pass, see Figure \ref{fig:Webshop_5}          \\ \hline
    ...
    TC-015                & Select payment method - PayPal             & PayPal payment options are selected and processed.        & Pass, see Figure \ref{fig:Webshop_6}          \\ \hline
...
\begin{itemize}
    \item The base project for the front end used vanilla JavaScript and no state management library. Initially to get the store up and running fast is perfect.
          But, once we need to add more functionality and the state management becomes more complex, we could have benefited from the inclusion of a stronger type system. And a dedicated state management solution.
          Even in the first stages of integration, the lack of types makes it more difficult to integrate with the backend.
    \item The choice of 3 different services to run the application might have been more complex than what was needed.
          In theory, either Azure App Service or Static Web apps could have easily handled the entirety of the application.
    \item Once we start scaling up, having three different services scaling independently even though is more flexible, could result in bigger expenses,
...
In general, the configuration of the services was simple enough. Nevertheless, there were some setbacks associated with the type of subscription we were using.
All services have auto-scaling capabilities, which can be enabled on demand, and configured according to the application's needs.
...
Nevertheless, this service can be upgraded to use "enterprise-grade edge" meaning that Azure will automatically cache the website on edge servers making it fast and easy to deliver to clients\ref{azurestaticwebapps}.
...
One of the disadvantages is that even though it allows for integration options with APIs and even databases, it is clear that we will have to architect the application to only be tailored to use these services under Azyres paradigms.

Another disadvantage is that it requires its resource group and own subnet.\\
\begin{figure}[H]
    \centering
...
This service was used due to its versatility in deploying any type of web application\ref{MicrosoftAzurePythonConfig}. To avoid extra costs associated with a private image registry,
to handle deploys we are packaging all the code and artifacts in the GitHub runner and publishing to our application.

One of the disadvantages is the price, once we start using more advanced features and more computing, the service scales rapidly in cost\ref{AppServiceplan}.\\
...
This is one of the more reasonable options to deploy a database server in Azure, given that the Free tier is quite capable\ref{azuredatabase}. It also offers automatic maintenance and backups.
...
\begin{itemize}
    \item The frontend application uses a really basic component-driven approach and segregates all API communication logic to its folder.
    ...
    \item In general this application is still limited by the scale of the database, given that we are not using a microservice approach or have a database that supports sharding.
          Nevertheless, we feel that for a webshop that needs to handle orders, transaction integrity is more important than data replication. Furthermore, given the flexible nature of the cloud, we could scale the server vertically if there is a need.
...
\begin{enumerate}
    \item \textbf{Branching Strategy:} \newline
          ...
          The code should be reviewed and tested before pushing to the main branch.
...
The team also implemented a database migration system to track changes to the tables and to seed the initial data without much effort.
...
Since all the applications will be tailored to a single service.


\subsection{Future Enhancements}
For production readiness, a lot of polish work is needed, especially in order handling and tracking, not to mention security and user management.
Nevertheless, on the infrastructure side, it is a matter of changing the SKUs and autoscaling configurations to something more appropriate depending on the expected load.
...
If a future cost assessment reveals that the choice of service was expensive, we could at least consolidate the presentation and business logic layers into one service.
...
\begin{thebibliography}{9}
    ...
    \bibitem{Webshop}
    DebasmitaMallick,
    \emph{React Webshop Repository},
    2024, Available:
    \url{https://github.com/DebasmitaMallick/React-e-Commerce-Website}.
